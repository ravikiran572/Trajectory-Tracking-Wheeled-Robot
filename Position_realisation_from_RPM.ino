...
void loop() {
    ...
    if (currentMillis - prevMillis >= 100) {  // Every 100 ms
      ...
      float lin_vel = (2 * PI/ 60) * r * (filteredRPM + filteredRPM1) / 2;
      float ang_vel = (2 * PI / 60) * r * (filteredRPM - filteredRPM1) / L;
      ...
      float controlOutput = computePID(desiredRPM, filteredRPM, prevError, integral);
      float controlOutput1 = computePID1(desiredRPM1, filteredRPM1, prevError1, integral1);
